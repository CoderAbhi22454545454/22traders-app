...
const tradeValidation = [
  ...
  body('result').optional().isIn(['win', 'loss', 'be', 'break even']).withMessage('Result must be win, loss, or be'),
...
router.post('/', upload.array('screenshots', 10), tradeValidation, async (req, res) => {
  try {
    ...
    const trade = new Trade({
      ...
      result: result === 'break even' ? 'be' : result, // Normalize to 'be' for consistency
      tradeOutcome,
      direction,
